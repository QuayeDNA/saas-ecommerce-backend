...
async function creditProfit(order) {
  ...
  if (order.storefrontData?.storefrontId) {
    const sf = await AgentStorefront
      .findById(order.storefrontData.storefrontId)
      .select("agentId")
      .lean();
  ...
  return { credited: true, agentId: agentId.toString(), markup, newBalance: updatedUser.earningsBalance };
}
...
async function run() {
  ...
  let credited = 0, skipped = 0, failed = 0, totalCredited = 0;
  ...
  for (const order of orders) {
    try {
      ...
      if (result.credited) {
        ...
        console.log(`  ✓ ${order.orderNumber.padEnd(15)} GH₵${result.markup.toFixed(2).padStart(6)} → agent ${result.agentId} (balance now GH₵${result.newBalance.toFixed(2)})`);
      } else {
        skipped++;
        console.log(`  - ${order.orderNumber.padEnd(15)} skipped (${result.reason})`);
      }
    } catch (err) {
      failed++;
  ...
  console.log(`\n${"─".repeat(55)}`);
  console.log(`Credited: ${credited} orders  |  GH₵${totalCredited.toFixed(2)} total`);
