...
class PaystackController {
    ...
    try {
      ...
      if (typeof paystackService.ensureKeys === "function") {
        try {
          ...
          logger.warn("[Paystack Webhook] ensureKeys failed", {
      ...
      if (!paystackService.verifyWebhookSignature(rawBody, signature)) {
        ...
        if (!paystackService.isConfigured()) {
          logger.error(
            "[Paystack Webhook] secret key not configured - webhook cannot be verified",
          );
        }
      ...
      if (!event && rawBody) {
        try {
          ...
          logger.error("[Paystack Webhook] failed to parse raw body", {
      ...
      switch (event.event) {
        ...
        case "transfer.failed":
          ...
          break;
        default:
          logger.info("[Paystack Webhook] unhandled event", {
            event: event.event,
          });
      }
  ...
  async handleChargeSuccess(event) {
    ...
    logger.warn("[Paystack Webhook] charge.success with unknown metadata", {
  ...
  async handleChargeFailed(event) {
    ...
    logger.info("[Paystack Webhook] charge.failed", {
      reference: data.reference,
    ...
    if (metadata.orderId) {
      ...
      if (order) {
        ...
        order.metadata.paystackFailure = {
          reference: data.reference,
          reason: data.gateway_response,
          failedAt: new Date(),
        };
