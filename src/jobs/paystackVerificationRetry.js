...
async function processTask(task) {
  ...
  try {
    ...
    if (!paystackData || paystackData.status !== 'success') {
      const reason = paystackData?.message || 'Paystack transaction not successful';
      logger.warn(`[PaystackVerificationJob] Reference ${reference} not successful: ${reason}`);
      ...
      if (task.attemptCount >= MAX_ATTEMPTS) {
        ...
        await task.save();
        logger.warn(`[PaystackVerificationJob] Marked task failed after ${task.attemptCount} attempts: ${reference}`);
      } else {
        task.status = 'pending';
        task.lastError = reason;
        await task.save();
    ...
    if (task.kind === 'storefront') {
      try {
        ...
        logger.error(`[PaystackVerificationJob] Error processing storefront payment for ${reference}: ${err.message}`);
        throw err;
      }
      ...
      try {
        ...
        logger.error(`[PaystackVerificationJob] Error processing wallet top-up for ${reference}: ${err.message}`);
        throw err;
      }
    }
    ...
    task.lastError = null;
    await task.save();
    logger.info(`[PaystackVerificationJob] Successfully processed Paystack reference ${reference}`);
  } catch (err) {
    ...
    await task.save();
    logger.error(`[PaystackVerificationJob] Error processing task ${reference}: ${err.message}`);
  }
}
      ...
      try {
        ...
        for (const t of tasks) {
          try {
            ...
            logger.error(`[PaystackVerificationJob] Failed to claim/process task ${t._id}: ${err.message}`);
          }
        }
      } catch (err) {
