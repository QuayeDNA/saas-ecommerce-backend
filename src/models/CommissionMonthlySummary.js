...
commissionMonthlySummarySchema.index({ tenantId: 1, year: -1, monthNumber: -1 });
commissionMonthlySummarySchema.index({ month: 1, agentId: 1 }, { unique: true });
...
commissionMonthlySummarySchema.methods.updatePaymentStatus = function() {
  if (this.totalEarned === 0) {
    ...
    this.paymentPercentage = Math.round((this.totalPaid / this.totalEarned) * 100);
  } else {
    this.paymentStatus = 'unpaid';
    this.paymentPercentage = 0;
  }
...
commissionMonthlySummarySchema.statics.findOrCreateForMonth = async function(agentId, tenantId, year, monthNumber) {
...
commissionMonthlySummarySchema.statics.getAgentSummaries = async function(agentId, options = {}) {
  const { 
...
commissionMonthlySummarySchema.statics.getTenantSummaries = async function(tenantId, options = {}) {
  const { 
    limit = 100,
    paymentStatus,
    month
...
const CommissionMonthlySummary = mongoose.model('CommissionMonthlySummary', commissionMonthlySummarySchema);
