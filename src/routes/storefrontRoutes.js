...
// ── Multer config for storefront asset uploads ──────────────────────────────
...
const validateBusinessName = param("businessName")
  ...
  .matches(/^[a-zA-Z0-9_-]+$/)
  .withMessage(
    "Business name must be 3-50 characters, alphanumeric with underscores/hyphens only",
  );
...
const validateStorefrontData = [
  ...
  body("paymentMethods.*.type")
    ...
    .withMessage(
      "Payment method type must be mobile_money, bank_transfer, or paystack",
    ),
  body("paymentMethods.*").custom((pm) => {
    if (pm.type === "mobile_money") {
      if (!pm.details?.accounts || !Array.isArray(pm.details.accounts)) {
        throw new Error(
          "Mobile money payment method must include an accounts array",
        );
      }
      ...
      pm.details.accounts.forEach((acc, i) => {
        if (
          !acc.provider ||
          !["MTN", "Vodafone", "AirtelTigo"].includes(acc.provider)
        ) {
          throw new Error(
            `Account ${i + 1}: provider must be MTN, Vodafone, or AirtelTigo`,
          );
        }
        ...
        if (!acc.accountName || acc.accountName.trim().length < 2) {
          throw new Error(
            `Account ${i + 1}: account name must be at least 2 characters`,
          );
        }
      });
...
const validateOrderData = [
  ...
  body("items.*.bundleId").isMongoId().withMessage("Invalid bundle ID"),
  body("items.*.quantity")
    .isInt({ min: 1 })
...
const validatePricingData = [
  ...
  body("pricing.*.bundleId").isMongoId().withMessage("Invalid bundle ID"),
...
const validateBundleToggle = [
  ...
  body("bundles.*.bundleId").isMongoId().withMessage("Invalid bundle ID"),
...
router.get(
  ...
  storefrontController.getRandomStorefronts,
);
...
router.put(
  ...
  [
    ...
    body("description")
      .optional()
      .isLength({ max: 500 })
      .withMessage("Description under 500 characters"),
    body("contactInfo.phone")
      .optional()
      .matches(/^[0-9+\-\s()]+$/)
...
// Storefront asset uploads (logo / banner) — replaces old file automatically
router.post(
  "/agent/storefront/logo",
  authenticate,
  (req, res, next) => {
  ...
  storefrontController.uploadLogo,
);
router.delete(
  "/agent/storefront/logo",
  authenticate,
  storefrontController.deleteLogo,
);
router.post(
  "/agent/storefront/banner",
  authenticate,
  (req, res, next) => {
    upload.single("file")(req, res, (err) => {
      if (err) {
  ...
  storefrontController.uploadBanner,
);
router.delete(
  "/agent/storefront/banner",
  authenticate,
  storefrontController.deleteBanner,
);
...
router.put(
  ...
  [
    ...
    body("notes")
      .optional()
      .isLength({ max: 500 })
      .withMessage("Notes under 500 characters"),
  ],
...
router.put(
  ...
  [
    ...
    body("reason")
      ...
      .withMessage("Rejection reason required (max 500 characters)"),
  ],
...
router.put(
  ...
  [
    ...
    body("reason")
      .optional()
      .isLength({ max: 500 })
      .withMessage("Reason under 500 characters"),
  ],
...
router.get(
  ...
  [
    query("ref")
      ...
      .withMessage("ref query parameter is required"),
  ],
