...
class SettingsService {
  ...
  async toggleSiteStatus() {
    ...
    logger.info(`Site status toggled to: ${settings.isSiteOpen ? 'open' : 'closed'}`);
  ...
  async getApiSettings() {
    ...
    return {
      ...
      telecelApiKey: settings.telecelApiKey || process.env.TELECEL_API_KEY || "",
      airtelTigoApiKey: settings.airtelTigoApiKey || process.env.AIRTELTIGO_API_KEY || "",
      apiEndpoint: settings.apiEndpoint || process.env.API_ENDPOINT || "https://api.telecomsaas.com"
    };
  }
  ...
  async changeUserRole(userId, newRole) {
    try {
      ...
      // Validate role
      const validRoles = ['customer', 'agent', 'admin', 'super_admin'];
